...
class DiffusionModel(nn.Module):
    def __init__(self, extractor, total_steps, time_embedding_dim=64, eta=0):
        ...
        for i in range(1, total_steps + 1):
            sigma[i] = ((1 - alpha_bar[i - 1]) / (1 - alpha_bar[i])) * beta[i]
    ...
    @torch.no_grad()
    def forward_determenistic_process(self, x0):
        self.eval()
        x_cur = x0
        latent = None
        ...
        for t in tqdm(range(self.total_steps)):
            ...
            alpha_tp1 = self.alpha_bar[t + 1]
            timesteps = torch.ones(x0.size(0), device=self.alpha.device, dtype=torch.int64) * t
            time_emb = self.get_time_embeddings(self.time_embedding_dim, timesteps)
            et, (latent, _, _) = self.extractor(x0, x_cur, time_emb,
                                                return_features=False,
                                                z=latent,
                                                deterministic_latent=True)
            x0_pred = (x_cur - et * (1 - alpha_t).sqrt()) / alpha_t.sqrt()
            x_cur = alpha_tp1.sqrt() * x0_pred + (1 - alpha_tp1).sqrt() * et

        return x_cur

    @torch.no_grad()
    def sample(self, x_current=None, x0=None, batch_size=8,
               track_trajectory=False, skip=1, latent=None):

        if x0 is not None:
            if x_current is None:
                x_current = self.forward_determenistic_process(x0)
